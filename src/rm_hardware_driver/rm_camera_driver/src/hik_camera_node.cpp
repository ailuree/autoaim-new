...
public:
  explicit HikCameraNode(const rclcpp::NodeOptions & options) : Node("hik_camera", options)
  {
    ...
    RCLCPP_INFO(this->get_logger(), "后加的-----------------Starting HikCameraNode!");//后加
    ...
    auto camera_info_url =
      this->declare_parameter("camera_info_url", "package://rm_bringup/config/camera_info.yaml");//改
    ...
    capture_thread_ = std::thread{[this]() -> void {
      ...
      while (rclcpp::ok()) {
        ...
        if (MV_OK == nRet) {
          ...
          camera_info_.header = image_msg_.header;
  ...
  if (enable_recorder) {
    ...
    recorder_ = std::make_unique<fyt::camera_driver::Recorder
>(
...
private:
  ...
  int auto_white_balance_;
      // Recorder
  std::unique_ptr<fyt::camera_driver::Recorder> recorder_;
